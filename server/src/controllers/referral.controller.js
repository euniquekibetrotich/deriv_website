...
export async function trackReferral(req, res, next) {
  try {
    ...
    const affiliate = await User.findOne({ where: { referralCode } });
    if (!affiliate) {
    ...
    return res.status(201).json({ referralId: referral.id, message: 'Referral tracked' });
  } catch (error) {
    next(error);
  }
}
...
export async function convertReferral(req, res, next) {
  try {
    ...
    if (!referralId || !referredEmail) {
      return res.status(400).json({ message: 'Referral ID and referred email are required' });
    }
